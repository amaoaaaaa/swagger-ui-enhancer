...
(function () {
    ...
    function useBaseUrlEnhancer() {
        ...
        window.fetch = async function (...args) {
            let [resource, config] = args as [string, RequestInit];

            if (resource.includes(configBaseUrl)) {
                const searchValue = resource.slice(
                    0,
                    resource.indexOf(configBaseUrl) + configBaseUrl.length
                );
    ...
    function useApiDocsHistory() {
        ...
        window.fetch = async function (...args) {
            return await originalFetch(...args).then(async (rep) => {
                const res = (await rep.clone().json()) as ApiDocs;
                ...
                if (res.swagger && res["x-openapi"]) {
                    ...
                    apiDocsHistoryData.forEach((historyItem, index) => {
                        ...
                        if (diff) {
                            console.log(
                                `%c --------- 文档更新 ${historyItem.diffTime} ---------`,
                                "color: #39f; font-size: 16px; font-weight: bold"
                            );
                            ...
                            diff.forEach((diffItem) => {
                                ...
                                const api = diffItem.path?.[1] as string;
        ...
        function diffApiDocs(apiDocsPrevVersion: ApiDocs | undefined, apiDocsRes: ApiDocs) {
        ...
        function getApiDocsHistoryData() {
            try {
                ...
                return dataStr ? (JSON.parse(dataStr) as ApiDocsHistory) : [];
            } catch (error) {
         ...
         * @param apiDocsRes
         */
        function saveApiDocsData(apiDocsRes: ApiDocs) {
            // 取出本地数据
            const apiDocsHistoryData = getApiDocsHistoryData();
    ...
    function useCopyRequestEnhancer() {
        ...
        function parseHandler() {
            ...
            document.querySelectorAll("div.opblock").forEach((el) => {
                // 接口简介
                const apiSummary = el.querySelector<HTMLDivElement>(".opblock-summary");

                // 处理过了，跳过
                if (!apiSummary || apiSummary.querySelector(".clipboard-btn")) return;
...
`;
                ...
                // 添加按钮
                const btnWrap = apiSummary.querySelector<HTMLDivElement>(
    ...
    function useTypeParseEnhancer() {
        ...
        function parseApiParametersType() {
            // 遍历所有的接口参数
            document.querySelectorAll<HTMLDivElement>(".parameters-container").forEach((el) => {
                const $el = $(el);
                const apiWrap = $el.parents(".opblock");
                const btnWrap =
                    el.previousElementSibling?.querySelector<HTMLHeadingElement>(".opblock-title");

                // 处理过了，跳过
                if (!btnWrap || btnWrap.querySelector(".clipboard-btn")) return;

                // 获取参数表格
                const table = el.querySelector<HTMLTableElement>("table.parameters");
                if (!table) return;
...
type ${typeName} = {${parseApiParametersTypeProp(table)}
         ...
         */
        function parseApiParametersTypeProp(tableEl: HTMLTableElement) {
            let str = "";

            tableEl.querySelectorAll<HTMLTableRowElement>("tbody tr").forEach((tr) => {
                /**
                 * 属性名
                 */
                const propName = tr
                 ...
                 * 可选的属性
                 */
                const isOptional = !Array.from(
                    tr.querySelector(".parameter__name")!.classList
                ).includes("required");

                /**
                 * 属性类型
                 */
                const propType = parserPropTypeText(
                    tr.querySelector(".parameter__type")!.textContent
                );
                 ...
                 */
                const propRemark =
                    tr.querySelector(".parameters-col_description .markdown")?.textContent ||
                    propName;
    ...
    ${propName}${isOptional ? "?" : ""}: ${propType};`;
        ...
        function parseModelsDefined() {
            ...
            document.querySelectorAll("span.model").forEach((span) => {
                // 处理过了，跳过
                if (span.querySelector(".clipboard-btn")) return;

                // 获取类型属性表格，没有展开的时候表格没渲染
                const table = span.querySelector<HTMLTableElement>("table.model");
                if (!table) return;
...
type ${typeName} = {${parseModelProp(table)}
         ...
         * @param {HTMLTableElement} tableEl 表格元素
         */
        function parseModelProp(tableEl: HTMLTableElement) {
            let str = "";

            tableEl.querySelectorAll<HTMLTableRowElement>("tr.false").forEach((tr) => {
                 ...
                 */
                const propType = parserPropTypeText(
                    tds[1].querySelector(".prop-type")?.textContent
                );
    ...
    ${propName}${isOptional ? "?" : ""}: ${propType};`;
         ...
         * 处理类型转换
         */
        function parserPropTypeText(propTypeText: string | undefined | null) {
            const typeMap = {
                string: "string",
                number: "number",
                integer: "number",
     ...
     * @returns 按钮dom对象
     */
    function createClipboardBtn(
        btnText: string,
        clipboardText: string,
        backgroundColor: string = "#39f"
    ) {
        // 创建复制按钮
     ...
     * 字符串首字母大写
     */
    function capitalizeFirstLetter([first, ...rest]: string) {
